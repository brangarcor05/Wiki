...
# 18. Qué se puede desarrollar a continuación
...
También se puede utilizar Maven si está instalado:
...
https://github.com/brangarcor05/Wiki
...
Si alguien nuevo entra al proyecto, debe entender primero:

1. `model/` → qué datos maneja la Wiki.
2. `service/` → de dónde obtiene esos datos.
3. `controller/` → qué ruta utiliza cada funcionalidad y qué datos envía a la vista.
4. `templates/` → cómo se presenta la información con Thymeleaf.
5. `templates/fragments/` → componentes HTML reutilizables.
6. `static/css/` → estilos.
7. `static/js/` → comportamiento del navegador y validaciones.
8. `application.properties` → configuración de Spring Boot.
9. `pom.xml` → dependencias y configuración Maven.
